...
/// procedure (SeasonService) and the live season views (StatsService), so frozen results and live
/// standings agree by construction.
/// </summary>
internal static class SeasonAggregates
{
    internal sealed class ParticipantAggregate
    {
    ...
    internal static Dictionary<int, ParticipantAggregate> ComputeParticipants(IEnumerable<Match> chronologicalMatches)
    {
    ...
    internal static Dictionary<(int Player1Id, int Player2Id), (int Matches, int Wins, int TotalScore)> ComputePairs(IEnumerable<Match> matches)
    {
